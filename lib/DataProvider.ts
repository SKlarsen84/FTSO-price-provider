...
let conf: DataProviderConfiguration = JSON.parse(
  fs.readFileSync(args['config']).toString()
) as DataProviderConfiguration
...
class DataProvider {
  ...
  async submitPriceHashes(lst: DataProviderData[]) {
    ...
    for (let p of lst) {
      ...
      if (price) {
        ...
        this.logger.info(
          `${p.label} | Submitting price: ${(preparedPrice / 10 ** p.decimals).toFixed(p.decimals)} $ for ${epochId}`
        )
  ...
  async setupSubmissionAndReveal() {
    ...
    if (diffSubmit > submitPeriod - conf.submitOffset && this.ftso2symbol.size >= this.ftsosCount) {
      setTimeout(() => {
        ...
        this.execute(async () => {
          await this.submitPriceHashes(this.data)
        })
  ...
  setupEvents() {
    this.priceSubmitterContract.on(
      ...
      async (submitter: string, epochId: any, ftsos: string[], hashes: string[], timestamp: any) => {
    ...
    this.priceSubmitterContract.on(
      'PricesRevealed',
      async (voter: string, epochId: any, ftsos: string[], prices: any[], randoms: string[], timestamp: any) => {
        ...
        for (let ftso of ftsos) {
          ...
          this.logger.info(
            `${p.label} | Price revealed in epoch ${epochIdStr}: ${(price / 10 ** p.decimals).toFixed(p.decimals)}$.`
          )
          ...
          if (priceInfo) {
            ...
            if (p) {
              this.logger.info(
                `${p.label} | Price that was submitted: ${(priceInfo.priceSubmitted / 10 ** 5).toFixed(5)}$`
              )
    ...
    this.ftsoContracts.forEach(contractWithSymbol => {
      contractWithSymbol.contract.on(
        ...
        async (
          ...
          this.logger.info(
            `Price finalized for ${contractWithSymbol.symbol} in epochId ${epochId}: price: ${(price / 10 ** 5).toFixed(
              5
            )}$,  finalization type: ${finalizationType}, rewarded: ${rewardedFtso}, low price: ${(
              lowRewardPrice /
              10 ** 5
            ).toFixed(5)}$, high price: ${(highRewardPrice / 10 ** 5).toFixed(5)}$, timestamp: ${timestamp.toString()}`
          )
        }
      )
    })
  ...
  async runDataProvider() {
    ...
    this.priceSubmitterWeb3Contract = await getWeb3Contract(
      this.web3,
      conf.priceSubmitterContractAddress,
      'PriceSubmitter'
    )
      ...
      try {
        ...
        for (let ftso of lst) {
          ...
          try {
            ...
            if (conf.whitelist) {
              try {
                var fnToEncode = this.voterWhitelisterContract.methods.requestWhitelistingVoter(
    ...
    try {
      ...
      this.epochSettings = new EpochSettings(
        bigNumberToMillis(data[0]),
        bigNumberToMillis(data[1]),
        bigNumberToMillis(data[2])
      )
